"""训练脚本 V2：支持强化 U-Net V2、组合损失、短图过采样、局部特征输入。

用法：
  python -m src.train_v2 --epochs 250 --base 32 --depth 4 --patch 256 \
      --batch 16 --lr 1e-3 --seed 0 \
      --out outputs/ckpts/v2_model_seed0.pt \
      --noise-aug
"""
...
def train(args):
    ...
    if args.short_factor > 1:
        ...
        print(f"[train_v2] oversample short factor={args.short_factor} -> {len(train_stems)} samples")
    ...
    val_ds = DocDataset(
        val_stems, train=False, with_features=True,
    )
    ...
    model = DenoiseUNetV2(in_channels=3, base=args.base, depth=args.depth).to(device)
    n_params = sum(p.numel() for p in model.parameters())
    print(f"[train_v2] params: {n_params:,}  device={device}")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if val_rmse < best_rmse - 1e-5:
            ...
            if no_improve >= patience:
                print(f"[train_v2] early stop at epoch {epoch}")
                break

    out_path = Path(args.out)
    out_path.parent.mkdir(parents=True, exist_ok=True)
    torch.save({"state_dict": best_state, "val_rmse": best_rmse, "args": vars(args)}, out_path)
    print(f"[train_v2] best val_rmse={best_rmse:.5f} saved to {out_path}")
    return best_rmse
...
if __name__ == "__main__":
    ...
    ap.add_argument("--out", type=str, default="outputs/ckpts/v2_model_seed0.pt")
    ap.add_argument("--short-factor", type=int, default=2, help="短图过采样倍数")
    ap.add_argument("--noise-aug", action="store_true", help="启用合成噪声增强")
